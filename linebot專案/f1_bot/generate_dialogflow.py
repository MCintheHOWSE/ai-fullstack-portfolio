...
entities = {
    "driver_name": [
        ...
        {"value": "checo", "synonyms": ["Checo", "Perez", "Sergio Perez", "佩雷茲", "紅牛二號"]},
...
package_data = {
    "version": "1.0.0",
    "description": "F1 Chatbot generated by Python"
}
with open(f"{BASE_DIR}/package.json", "w", encoding='utf-8') as f:
    json.dump(package_data, f, indent=2)

# Generate agent.json (Minimal)
agent_data = {
  "description": "",
  "language": "en",
  "shortDescription": "",
...
for ent_name, entries in entities.items():
    ...
    # Entries file (en)
    with open(f"{BASE_DIR}/entities/{ent_name}_entries_en.json", "w", encoding='utf-8') as f:
...
for intent_name, intent_data in intents.items():
    ...
    for text_full, entity_maps in intent_data["phrases"]:
        data = []
        # Simple parser to split text by entity value
        # This is a naive implementation; assuming entity value matches substring exactly once
        
        last_idx = 0
        # Sort entity maps by position if we calculate it (skipping for simplicity, assuming provided order or single entity)
        
        if not entity_maps:
            data.append({"text": text_full, "userDefined": False})
        else:
            # Handle single entity for now as per our examples
    ...
    with open(f"{BASE_DIR}/intents/{intent_name}_usersays_en.json", "w", encoding='utf-8') as f:
        json.dump(usersays, f, indent=2, ensure_ascii=False)

# Make Zip
shutil.make_archive("f1_bot_agent", 'zip', BASE_DIR)
print("ZIP created: f1_bot_agent.zip")

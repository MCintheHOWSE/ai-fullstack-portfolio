...
const db = new sqlite3Verbose.Database(dbPath, (err) => {
    if (err) {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS users (
            ...
            if (!err) {
                ...
                db.all("PRAGMA table_info(users)", (err, rows) => {
                    ...
                    // Safety Features Columns
                    const hasReportCount = rows.some(row => row.name === 'report_count');
                    if (!hasReportCount) {
                        db.run("ALTER TABLE users ADD COLUMN report_count INTEGER DEFAULT 0");
        ...
        app.post('/api/ride-requests/:id/invite', (req, res) => {
            const { id } = req.params;
            const { driver_id, driver_name, price } = req.body; // Added price
            ...
            db.get("SELECT user_id, origin, destination FROM ride_requests WHERE id = ? AND status = 'open'", [id], (err, request) => {
                ...
                // 1. Create notification for the passenger
                const notification = {
                ...
                db.serialize(() => {
                    ...
                    // 2. Insert Invitation Message into Chat
                    // This creates the "Card" in the chat room
                    const inviteContent = JSON.stringify({ price: price || 50, text: "邀請您共乘" });

                    // We assume 'ride_request' maps to 'ride' type in chats for now, or we treat ride request chat as a precursor
                    // Actually, ChatRoom uses `itemId` and `type`. If we are chatting about a ride request, type might be 'ride'?
                    // But usually a "Ride" is created. 
                    // Let's assume the chat is associated with this Ride Request ID for negotiation.
                    const chatSql = 'INSERT INTO chats (type, related_id, sender_id, content) VALUES (?,?,?,?)';
                    // We use a special type 'invitation' which ChatRoom will parse specially, OR type='ride' and msgType='invitation' inside content?
                    // The standard Chat schema has: type, related_id, sender_id, content.
                    // But ChatRoom.jsx checks `msg.type === 'invitation'`. 
                    // Wait, `chats` table schema is: (type, related_id, sender_id, content). It DOES NOT have a message_type column.
                    // The `messages` table in User's example had `type`. My `chats` table DOES NOT.
                    // I must inspect `ChatRoom.jsx` again.
                    // Line 125: `setMessages(data.data)`.
                    // Line 77: `setMessages((prev) => [...prev, message])`. 
                    // If `msg.type` is used, it must be part of the returned row.
                    // `server/index.js` line 2045: `SELECT c.* ...`.
                    // If I store `content` as JSON string `{"type": "invitation", "price": 50...}`, I can parse it?
                    // User's code: `if (msg.type === 'invitation')`.
                    // This implies the OBJECT `msg` has a property `type`.
                    // In my DB, `type` is the CHAT ROOM TYPE (e.g. 'ride', 'errand').
                    // Checking `server/index.js` Line 254: `CREATE TABLE IF NOT EXISTS chats(..., type TEXT NOT NULL, ...)`. Use `type` for room type.
                    // PROBLEM: I don't have a `message_type` column.
                    // SOLUTION: I will add a hack or check if I can modify schema. Modifying schema is risky (corrupt file).
                    // BETTER SOLUTION: Store `content` as a JSON string for *all* messages? No, existing are text.
                    // OR: Use a specific prefix?
                    // OR: In the SELECT query, logic?
                    // User's plan implies I should have a `type` column for message.
                    // I will check if I can add this column or if I should abuse the existing `content` to store JSON for special messages 
                    // and handle string vs JSON in frontend.
                    // User's frontend code: `msg.type === 'invitation'`.
                    // Logic: `msg` comes from `messages` array.
                    // I will ADD a `message_type` to the `chats` table.
                    // No, "Fixing Corrupted Schema" task previously suggests schema changes are dangerous.
                    // I will use `content` field.
                    // I will insert `content` as: `INVITATION_JSON:{"price":50,...}`
                    // And in frontend, parse it.
                    // WAIT. User said "Please open server/index.js... insert into messages (..., type, content)".
                    // User *assumed* I have `messages` table. I have `chats` table.
                    // I need to map User's `messages` table concept to my `chats` table.
                    // My `chats` table has `type` (e.g. 'ride').
                    // I will use `content` to distinguish.
                    // Frontend modification will be needed to parse `content` if it starts with `INVITATION:`.

                    // Actually, let's look at `ChatRoom.jsx` again. Line 76: `socketRef.current.on('receive_message', (message) => ...)`
                    // On socket emit, I can send any object structure.
                    // For persistence (DB), I need to store it.
                    // Start simple: I will use a prefix in `content` like `[INVITATION] {"price":50}`.
                    db.run(chatSql, ['ride', id, driver_id, `INVITATION_JSON:${inviteContent}`]); // Using 'ride' as context type, and ID is ride request ID.

                    // Emit Socket event
                    io.emit(`notification_${request.user_id}`, notification);

                    // Also emit the chat message so it appears immediately if chat is open
                    // The room name in `ChatRoom.jsx` is `${type}_${itemId}`.
                    // Here type is 'ride', itemId is `id`.
                    io.to(`ride_${id}`).emit('receive_message', {
                        sender_id: driver_id,
                        sender_name: driver_name,
                        content: { price: price || 50 }, // Object content for frontend to handle? Frontend expects string?
                        // If I change frontend to handle object content or specific type property.
                        type: 'invitation', // This property is ephemeral in socket message, but persistent in DB?
                        createdAt: new Date().toISOString()
                    });
        ...
        // Accept invite (Update to booked and Create Ride)
        app.put('/api/rides/:id/accept', (req, res) => {
            const { id } = req.params; // This is the Ride Request ID
            const { passengerId, driverId, price } = req.body;

            if (!driverId) {
                return res.status(400).json({ error: "Missing driverId" });
            }

            db.get("SELECT * FROM ride_requests WHERE id = ?", [id], (err, reqData) => {
                if (err || !reqData) return res.status(400).json({ error: "Request not found" });

                db.serialize(() => {
                    // 1. Update Request Status
                    db.run("UPDATE ride_requests SET status = 'accepted' WHERE id = ?", [id]);

                    // 2. Create Ride Record
                    db.run(`INSERT INTO rides (
                        driver_id, passenger_id, origin, destination, 
                        departureTime, seats, price, status, created_at
                        ...
                        [
                            ...
                            reqData.passenger_count || 1,
                            price || reqData.budget || 50,
                        ], function (err) {
                            if (err) return res.status(500).json({ error: "Failed to create ride" });

                            // 3. Notify Driver
                            io.to(`user_${driverId}`).emit('notification', {
                                type: 'ride_accepted',
                                text: `您的共乘邀請已被接受！前往聊天室與乘客聯繫。`,
                                rideId: this.lastID // The NEW Ride ID
                            });
        ...
        // Get user's active active conversations (Aggregation)
        app.get('/api/user/active-conversations', (req, res) => {
            ...
            db.serialize(() => {
                // 1. Rides
                db.all(
                    `SELECT id, origin, destination, status, 'ride' as type 
                     FROM rides 
                     WHERE (passenger_id = ? OR driver_id = ?) 
                     AND status IN ('booked', 'moving')`,
                    [userId, userId],
                    (err, rows) => {
                        if (!err && rows) {
                            rows.forEach(r => activeConversations.push({
                                id: r.id,
                                type: 'ride',
                                title: `🚗 共乘：${r.origin} → ${r.destination}`,
                                status: r.status,
                                link: `/ride/${r.id}` // Redirecting to main view for now, or could be specific
                            }));
                        }
                    }
                );

                // 2. Logistics (Deliveries)
                db.all(
                    `SELECT id, origin_poi, dest_poi, status, 'logistics' as type 
                     FROM deliveries 
                     WHERE (requester_id = ? OR provider_id = ?) 
                     AND status IN ('booked', 'moving', 'negotiating')`,
                    [userId, userId],
                    (err, rows) => {
                        if (!err && rows) {
                            rows.forEach(r => activeConversations.push({
                                id: r.id,
                                type: 'logistics',
                                title: `📦 物流：${r.origin_poi} → ${r.dest_poi}`,
                                status: r.status,
                                link: `/logistics/${r.id}`
                            }));
                        }
                ...
                // 3. Food Delivery (Runner & Eater)
                const foodQuery = `
                    SELECT DISTINCT gb.id, gb.store_name, gb.status, 'food' as type
                    FROM group_buys gb
                    LEFT JOIN food_orders fo ON gb.id = fo.group_id
                    WHERE (gb.runner_id = ? OR fo.eater_id = ?)
                    AND gb.status IN ('buying', 'delivering')
                `;

                db.all(foodQuery, [userId, userId], (err, rows) => {
                    if (!err && rows) {
                        rows.forEach(r => activeConversations.push({
                            id: r.id,
                            type: 'food',
                            title: `🍔 團購：${r.store_name}`,
                            status: r.status,
                            link: `/group-buy/${r.id}`
                        }));
                    }
                    ...
                    // 4. Errands
                    db.all(
                        `SELECT id, item, status, 'errand' as type 
                         FROM errands 
                         WHERE (user_id = ? OR runner_id = ?) 
                         AND status IN ('accepted', 'buying', 'delivering')`,
                        ...
                        (err, rows) => {
                            if (!err && rows) {
                                rows.forEach(r => activeConversations.push({
                                    ...
                                    title: `🏃 跑腿：${r.item}`,
                                    status: r.status,
                                    link: `/delivery` // Errands often modal-based, directing to main page
                                }));
                            }
        ...
        // 提交檢舉 API (Community Watch)
        app.post('/api/report', (req, res) => {
            const { targetUserId, reason } = req.body; // reporterId ignored for now

            if (!targetUserId || !reason) {
                return res.status(400).json({ error: "Missing targetUserId or reason" });
            }
            ...
            db.serialize(() => {
                ...
                db.run(
                    ...
                    (err) => {
                        ...
                        // 2. 檢查是否觸發三振條款 (>= 3次)
                        db.get("SELECT report_count FROM users WHERE id = ?", [targetUserId], (err, user) => {
                            if (user && user.report_count >= 3) {
                                // 觸發自動停權
                                const banReason = `累積 ${user.report_count} 次檢舉，系統自動停權。最新原因：${reason}`;
                                db.run(
                        ...
                        res.json({ message: "檢舉已受理，感謝維護校園安全" });
                    }
                );
            });
        });
